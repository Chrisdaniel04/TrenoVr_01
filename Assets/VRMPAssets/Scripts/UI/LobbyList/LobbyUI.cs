...
namespace XRMultiplayer
{
    public class LobbyUI : MonoBehaviour
    {
        ...
        [SerializeField]
        public bool disableLobbyList = true;
        ...
        {
            EnableRefresh();
            if (m_UpdateLobbiesRoutine != null)
            {
                StopCoroutine(m_UpdateLobbiesRoutine);
                m_UpdateLobbiesRoutine = null;
            }
        }
        ...
        IEnumerator UpdateButtonCooldown()
        {
            ...
            {
                // Se durante la coroutine hai cambiato pannello/distrutto UI
                if (!HasRefreshUI() || !isActiveAndEnabled)
                    yield break;
        ...
        {
            if ((int)XRINetworkGameManager.CurrentConnectionState.Value < 2)
                return;

            // se non hai la UI refresh, salta il cooldown e vai avanti
            if (HasRefreshUI())
            {
                if (m_CooldownImage.enabled)
                    return;
        ...
        bool HasRefreshUI()
        {
            return m_RefreshButton != null && m_CooldownImage != null;
        }
    }
}
